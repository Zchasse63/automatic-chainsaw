...
export function WeekDayRow({ date, workouts, isToday, index, onAddWorkout }: WeekDayRowProps) {
  ...
  return (
    <motion.div
      ...
      className={`flex gap-3 p-3 rounded-2xl transition-all border min-h-[64px] ${
        isOver
        ...
        {workouts.length > 0 ? (
          ...
          <div className="flex items-center gap-2 py-3 min-h-[40px]">
            ...
            <button
              ...
              className="ml-auto w-7 h-7 rounded-full bg-white/5 flex items-center justify-center text-white/20 hover:text-white/40 hover:bg-white/10 transition-colors"
            >
              <Plus size={12} />
            </button>
          </div>
